...
<body>
    ...
    <section id="home">
        <div class="container mt-5">
            <h1>INSCRIPCIONES</h1>
            <a href="{{ route('inscriptions.create') }}" class="btn btn-primary">Crear nueva inscripción</a>
            <br><br>
            <table class="table">
                <tr>
                    <th>ID</th>
                ...
                @foreach ($inscriptions as $inscription)
                    <tr>
                        ...
                        <td>{{ $inscription->student->enrollment }}</td>
                        <td>{{ $inscription->student->name }} {{ $inscription->student->maternal_surname }}
                            {{ $inscription->student->paternal_surname }}</td>
                        <td>{{ $inscription->group->name }}</td>
                        <td>{{ $inscription->group->shift->name }}</td>
                        <td>{{ $inscription->group->semester->name }}</td>
                        <td>
